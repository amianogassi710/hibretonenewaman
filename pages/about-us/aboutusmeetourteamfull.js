export default function Aboutusmeetourteamfull() {
    return (
        <>
            <section className="section-box mt-75 mt-75-50">
                <div className="post-loop-grid">
                    <div className="container1">
                        <div className="text-center">
                            <div className="component-title wow animate__animated animate__fadeInUp" >
                                Meet Our Team
                            </div>
                            <div className="component-subtitle wow animate__animated animate__fadeInUp">
                                A world-class team, board members and advisors
                            </div>
                        </div>
                        <div className="row" style={{ marginTop: '35px' }}>
                            <div className="col-xl-3 col-lg-3 col-md-6 col-sm-6 col-12 mb5 meetourteamcard">
                                <div className="card-grid-4 text-center hover-up paddingaboutus">
                                    <div className="image-top-feature">
                                        <figure>
                                            <img alt="jobBox" src="../assets/imgs/page/about-us/Drew.jpg" />
                                        </figure>
                                    </div>
                                    <div className="card-grid-4-info-aboutus">
                                        <div className="left-content">
                                            <div className="meetourteamname">Drew Currie</div>
                                            <div className="positionaboutus">Founder</div>
                                        </div>
                                        <a className="share-linkedin social-share-link-aboutus" href="https://www.linkedin.com/in/drew-currie-7863b32b/">
                                        </a>
                                    </div>
                                </div>
                            </div>
                            <div className="col-xl-3 col-lg-3 col-md-6 col-sm-6 col-12 mb5 meetourteamcard">
                                <div className="card-grid-4 text-center hover-up paddingaboutus">
                                    <div className="image-top-feature">
                                        <figure>
                                            <img alt="jobBox" src="../assets/imgs/page/about-us/Nikki.JPG" />
                                        </figure>
                                    </div>
                                    <div className="card-grid-4-info-aboutus">
                                        <div className="left-content">
                                            <div className="meetourteamname">Nikki Salisbury</div>
                                            <div className="positionaboutus">Co-Founder</div>
                                        </div>
                                        <a className="share-linkedin social-share-link-aboutus" href="#">
                                        </a>
                                    </div>
                                </div>
                            </div>
                            <div className="col-xl-3 col-lg-3 col-md-6 col-sm-6 col-12 mb5 meetourteamcard">
                                <div className="card-grid-4 text-center hover-up paddingaboutus">
                                    <div className="image-top-feature">
                                        <figure>
                                            <img alt="jobBox" src="../assets/imgs/page/about-us/Kali.jpg" />
                                        </figure>
                                    </div>
                                    <div className="card-grid-4-info-aboutus">
                                        <div className="left-content">
                                            <div className="meetourteamname">Kali Bagarry</div>
                                            <div className="positionaboutus">CTO</div>
                                        </div>
                                        <a className="share-linkedin social-share-link-aboutus" href="#">
                                        </a>
                                    </div>
                                </div>
                            </div>
                            <div className="col-xl-3 col-lg-3 col-md-6 col-sm-6 col-12 mb5 meetourteamcard">
                                <div className="card-grid-4 text-center hover-up paddingaboutus">
                                    <div className="image-top-feature">
                                        <figure>
                                            <img alt="jobBox" src="../assets/imgs/page/about-us/Assim.jpg" />
                                        </figure>
                                    </div>
                                    <div className="card-grid-4-info-aboutus">
                                        <div className="left-content">
                                            <div className="meetourteamname">Assim Ishaque</div>
                                            <div className="positionaboutus">Finance</div>
                                        </div>
                                        <a className="share-linkedin social-share-link-aboutus" href="#">
                                        </a>
                                    </div>
                                </div>
                            </div>
                            <div className="col-xl-3 col-lg-3 col-md-6 col-sm-6 col-12 mb5 meetourteamcard">
                                <div className="card-grid-4 text-center hover-up paddingaboutus">
                                    <div className="image-top-feature">
                                        <figure>
                                            <img alt="jobBox" src="../assets/imgs/page/about-us/Monder.jpg" />
                                        </figure>
                                    </div>
                                    <div className="card-grid-4-info-aboutus">
                                        <div className="left-content">
                                            <div className="meetourteamname">Prof Monder Ram OBE</div>
                                            <div className="positionaboutus">Non-Executive Director</div>
                                        </div>
                                        <a className="share-linkedin social-share-link-aboutus" href="#">
                                        </a>
                                    </div>
                                </div>
                            </div>
                            <div className="col-xl-3 col-lg-3 col-md-6 col-sm-6 col-12 mb5 meetourteamcard">
                                <div className="card-grid-4 text-center hover-up paddingaboutus">
                                    <div className="image-top-feature">
                                        <figure>
                                            <img alt="jobBox" src="../assets/imgs/page/about-us/Pam.jpg" />
                                        </figure>
                                    </div>
                                    <div className="card-grid-4-info-aboutus">
                                        <div className="left-content">
                                            <div className="meetourteamname">Pam Sheemar</div>
                                            <div className="positionaboutus">Non-Executive Director</div>
                                        </div>
                                        <a className="share-linkedin social-share-link-aboutus" href="#">
                                        </a>
                                    </div>
                                </div>
                            </div>
                            <div className="col-xl-3 col-lg-3 col-md-6 col-sm-6 col-12 mb5 meetourteamcard">
                                <div className="card-grid-4 text-center hover-up paddingaboutus">
                                    <div className="image-top-feature">
                                        <figure>
                                            <img alt="jobBox" src="../assets/imgs/page/about-us/Mark.jpg" />
                                        </figure>
                                    </div>
                                    <div className="card-grid-4-info-aboutus">
                                        <div className="left-content">
                                            <div className="meetourteamname">Dr Mark Esho OBE</div>
                                            <div className="positionaboutus">Non-Executive Director</div>
                                        </div>
                                        <a className="share-linkedin social-share-link-aboutus" href="#">
                                        </a>
                                    </div>
                                </div>
                            </div>
                            <div className="col-xl-3 col-lg-3 col-md-6 col-sm-6 col-12 mb5 meetourteamcard">
                                <div className="card-grid-4 text-center hover-up paddingaboutus">
                                    <div className="image-top-feature">
                                        <figure>
                                            <img alt="jobBox" src="../assets/imgs/page/about-us/Patricia.jpg" />
                                        </figure>
                                    </div>
                                    <div className="card-grid-4-info-aboutus">
                                        <div className="left-content">
                                            <div className="meetourteamname">Patricia Hamzahee</div>
                                            <div className="positionaboutus">Non-Executive Director</div>
                                        </div>
                                        <a className="share-linkedin social-share-link-aboutus" href="#">
                                        </a>
                                    </div>
                                </div>
                            </div>

                            <div className="col-xl-3 col-lg-3 col-md-6 col-sm-6 col-12 mb5 meetourteamcard">
                                <div className="card-grid-4 text-center hover-up paddingaboutus">
                                    <div className="image-top-feature">
                                        <figure>
                                            <img alt="jobBox" src="../assets/imgs/page/about-us/Tiago.jpg" />
                                        </figure>
                                    </div>
                                    <div className="card-grid-4-info-aboutus">
                                        <div className="left-content">
                                            <div className="meetourteamname">Tiago Carvalho</div>
                                            <div className="positionaboutus">Operations</div>
                                        </div>
                                        <a className="share-linkedin social-share-link-aboutus" href="#">
                                        </a>
                                    </div>
                                </div>
                            </div>
                            <div className="col-xl-3 col-lg-3 col-md-6 col-sm-6 col-12 mb5 meetourteamcard">
                                <div className="card-grid-4 text-center hover-up paddingaboutus">
                                    <div className="image-top-feature">
                                        <figure>
                                            <img alt="jobBox" src="../assets/imgs/page/about-us/Ememobong.jpg" />
                                        </figure>
                                    </div>
                                    <div className="card-grid-4-info-aboutus">
                                        <div className="left-content">
                                            <div className="meetourteamname">Ememobong Etim</div>
                                            <div className="positionaboutus">Graphic Design</div>
                                        </div>
                                        <a className="share-linkedin social-share-link-aboutus" href="#">
                                        </a>
                                    </div>
                                </div>
                            </div>
                            <div className="col-xl-3 col-lg-3 col-md-6 col-sm-6 col-12 mb5 meetourteamcard">
                                <div className="card-grid-4 text-center hover-up paddingaboutus">
                                    <div className="image-top-feature">
                                        <figure>
                                            <img alt="jobBox" src="../assets/imgs/page/about-us/Ashimi.jpg" />
                                        </figure>
                                    </div>
                                    <div className="card-grid-4-info-aboutus">
                                        <div className="left-content">
                                            <div className="meetourteamname">Mayor Ashimi</div>
                                            <div className="positionaboutus">Business Analyst</div>
                                        </div>
                                        <a className="share-linkedin social-share-link-aboutus" href="#">
                                        </a>
                                    </div>
                                </div>
                            </div>
                            <div className="col-xl-3 col-lg-3 col-md-6 col-sm-6 col-12 mb5 meetourteamcard">
                                <div className="card-grid-4 text-center hover-up paddingaboutus">
                                    <div className="image-top-feature">
                                        <figure>
                                            <img alt="jobBox" src="../assets/imgs/page/about-us/Robel.jpg" />
                                        </figure>
                                    </div>
                                    <div className="card-grid-4-info-aboutus">
                                        <div className="left-content">
                                            <div className="meetourteamname">Robel Klefmariam</div>
                                            <div className="positionaboutus">Software Developer</div>
                                        </div>
                                        <a className="share-linkedin social-share-link-aboutus" href="#">
                                        </a>
                                    </div>
                                </div>
                            </div>

                            <div className="col-xl-3 col-lg-3 col-md-6 col-sm-6 col-12 mb5 meetourteamcard">
                                <div className="card-grid-4 text-center hover-up paddingaboutus">
                                    <div className="image-top-feature">
                                        <figure>
                                            <img alt="jobBox" src="../assets/imgs/page/about-us/Nick.jpg" />
                                        </figure>
                                    </div>
                                    <div className="card-grid-4-info-aboutus">
                                        <div className="left-content">
                                            <div className="meetourteamname">Nick Jenkins</div>
                                            <div className="positionaboutus">Advisor</div>
                                        </div>
                                        <a className="share-linkedin social-share-link-aboutus" href="#">
                                        </a>
                                    </div>
                                </div>
                            </div>
                            <div className="col-xl-3 col-lg-3 col-md-6 col-sm-6 col-12 mb5 meetourteamcard">
                                <div className="card-grid-4 text-center hover-up paddingaboutus">
                                    <div className="image-top-feature">
                                        <figure>
                                            <img alt="jobBox" src="../assets/imgs/page/about-us/Sarah.jpg" />
                                        </figure>
                                    </div>
                                    <div className="card-grid-4-info-aboutus">
                                        <div className="left-content">
                                            <div className="meetourteamname">Sarah Beaumont OBE</div>
                                            <div className="positionaboutus">Advisor</div>
                                        </div>
                                        <a className="share-linkedin social-share-link-aboutus" href="#">
                                        </a>
                                    </div>
                                </div>
                            </div>
                            <div className="col-xl-3 col-lg-3 col-md-6 col-sm-6 col-12 mb5 meetourteamcard">
                                <div className="card-grid-4 text-center hover-up paddingaboutus">
                                    <div className="image-top-feature">
                                        <figure>
                                            <img alt="jobBox" src="../assets/imgs/page/about-us/Tom.jpg" />
                                        </figure>
                                    </div>
                                    <div className="card-grid-4-info-aboutus">
                                        <div className="left-content">
                                            <div className="meetourteamname">Tom Adeyoola</div>
                                            <div className="positionaboutus">Advisor</div>
                                        </div>
                                        <a className="share-linkedin social-share-link-aboutus" href="#">
                                        </a>
                                    </div>
                                </div>
                            </div>
                            <div className="col-xl-3 col-lg-3 col-md-6 col-sm-6 col-12 mb5 meetourteamcard">
                                <div className="card-grid-4 text-center hover-up paddingaboutus">
                                    <div className="image-top-feature">
                                        <figure>
                                            <img alt="jobBox" src="../assets/imgs/page/about-us/Richard.jpg" />
                                        </figure>
                                    </div>
                                    <div className="card-grid-4-info-aboutus">
                                        <div className="left-content">
                                            <div className="meetourteamname">Richard Blakesley</div>
                                            <div className="positionaboutus">Advisor</div>
                                        </div>
                                        <a className="share-linkedin social-share-link-aboutus" href="#">
                                        </a>
                                    </div>
                                </div>
                            </div>

                            <div className="col-xl-3 col-lg-3 col-md-6 col-sm-6 col-12 mb5 meetourteamcard">
                                <div className="card-grid-4 text-center hover-up paddingaboutus">
                                    <div className="image-top-feature">
                                        <figure>
                                            <img alt="jobBox" src="../assets/imgs/page/about-us/Eric.jpg" />
                                        </figure>
                                    </div>
                                    <div className="card-grid-4-info-aboutus">
                                        <div className="left-content">
                                            <div className="meetourteamname">Eric Collins</div>
                                            <div className="positionaboutus">Advisor</div>
                                        </div>
                                        <a className="share-linkedin social-share-link-aboutus" href="#">
                                        </a>
                                    </div>
                                </div>
                            </div>
                            <div className="col-xl-3 col-lg-3 col-md-6 col-sm-6 col-12 mb5 meetourteamcard">
                                <div className="card-grid-4 text-center hover-up paddingaboutus">
                                    <div className="image-top-feature">
                                        <figure>
                                            <img alt="jobBox" src="../assets/imgs/page/about-us/Melissa.jpg" />
                                        </figure>
                                    </div>
                                    <div className="card-grid-4-info-aboutus">
                                        <div className="left-content">
                                            <div className="meetourteamname">Melissa Snover</div>
                                            <div className="positionaboutus">Advisor</div>
                                        </div>
                                        <a className="share-linkedin social-share-link-aboutus" href="#">
                                        </a>
                                    </div>
                                </div>
                            </div>
                            <div className="col-xl-3 col-lg-3 col-md-6 col-sm-6 col-12 mb5 meetourteamcard">
                                <div className="card-grid-4 text-center hover-up paddingaboutus">
                                    <div className="image-top-feature">
                                        <figure>
                                            <img alt="jobBox" src="../assets/imgs/page/about-us/Gordon.jpg" />
                                        </figure>
                                    </div>
                                    <div className="card-grid-4-info-aboutus">
                                        <div className="left-content">
                                            <div className="meetourteamname">Gordon Merrylees</div>
                                            <div className="positionaboutus">Advisor</div>
                                        </div>
                                        <a className="share-linkedin social-share-link-aboutus" href="#">
                                        </a>
                                    </div>
                                </div>
                            </div>
                            <div className="col-xl-3 col-lg-3 col-md-6 col-sm-6 col-12 mb5 meetourteamcard">
                                <div className="card-grid-4 text-center hover-up paddingaboutus">
                                    <div className="image-top-feature">
                                        <figure>
                                            <img alt="jobBox" src="../assets/imgs/page/about-us/Saloni.jpg" />
                                        </figure>
                                    </div>
                                    <div className="card-grid-4-info-aboutus">
                                        <div className="left-content">
                                            <div className="meetourteamname">Saloni Wadhwa</div>
                                            <div className="positionaboutus">Operations</div>
                                        </div>
                                        <a className="share-linkedin social-share-link-aboutus" href="#">
                                        </a>
                                    </div>
                                </div>
                            </div>

                            <div className="col-xl-3 col-lg-3 col-md-6 col-sm-6 col-12 mb5 meetourteamcard">
                                <div className="card-grid-4 text-center hover-up paddingaboutus">
                                    <div className="image-top-feature">
                                        <figure>
                                            <img alt="jobBox" src="../assets/imgs/page/about-us/Steven.jpg" />
                                        </figure>
                                    </div>
                                    <div className="card-grid-4-info-aboutus">
                                        <div className="left-content">
                                            <div className="meetourteamname">Steven Williams</div>
                                            <div className="positionaboutus">Advisor</div>
                                        </div>
                                        <a className="share-linkedin social-share-link-aboutus" href="#">
                                        </a>
                                    </div>
                                </div>
                            </div>
                            <div className="col-xl-3 col-lg-3 col-md-6 col-sm-6 col-12 mb5 meetourteamcard">
                                <div className="card-grid-4 text-center hover-up paddingaboutus">
                                    <div className="image-top-feature">
                                        <figure>
                                            <img alt="jobBox" src="../assets/imgs/page/about-us/Kenedy.jpg" />
                                        </figure>
                                    </div>
                                    <div className="card-grid-4-info-aboutus">
                                        <div className="left-content">
                                            <div className="meetourteamname">Kenedy Mullinge</div>
                                            <div className="positionaboutus">Software Developer</div>
                                        </div>
                                        <a className="share-linkedin social-share-link-aboutus" href="#">
                                        </a>
                                    </div>
                                </div>
                            </div>
                            <div className="col-xl-3 col-lg-3 col-md-6 col-sm-6 col-12 mb5 meetourteamcard">
                                <div className="card-grid-4 text-center hover-up paddingaboutus">
                                    <div className="image-top-feature">
                                        <figure>
                                            <img alt="jobBox" src="../assets/imgs/page/about-us/Men.jpg" />
                                        </figure>
                                    </div>
                                    <div className="card-grid-4-info-aboutus">
                                        <div className="left-content">
                                            <div className="meetourteamname">Software Developer</div>
                                            <div className="positionaboutus">University of Birmingham</div>
                                        </div>
                                        <a className="share-linkedin social-share-link-aboutus" href="#">
                                        </a>
                                    </div>
                                </div>
                            </div>
                            <div className="col-xl-3 col-lg-3 col-md-6 col-sm-6 col-12 mb5 meetourteamcard">
                                <div className="card-grid-4 text-center hover-up paddingaboutus">
                                    <div className="image-top-feature">
                                        <figure>
                                            <img alt="jobBox" src="../assets/imgs/page/about-us/Women.jpg" />
                                        </figure>
                                    </div>
                                    <div className="card-grid-4-info-aboutus">
                                        <div className="left-content">
                                            <div className="meetourteamname">Software Developer</div>
                                            <div className="positionaboutus">Aston University</div>
                                        </div>
                                        <a className="share-linkedin social-share-link-aboutus" href="#">
                                        </a>
                                    </div>
                                </div>
                            </div>






                        </div>
                    </div>
                </div>
            </section>
        </>
    );
}
